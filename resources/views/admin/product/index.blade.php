        ...
        <div class="card shadow mb-4">
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-bordered table" id="dataTable" width="100%" cellspacing="0">
                        ...
                        <tbody>
                            ...
                            <form action="{{ route('product.update', $pro->id) }}" method="POST">
                                ...
                                <tr>
                                    <td>{{ ++$i }}</td>
                                    <td><img width="50px" height="50px" src="/img/products/{{$pro->image}}" alt=""></td>
                                    ...
                                    <td>
                                        <select class='form-control' name="category_id">
                                            @foreach($cats as $cat)
                                            <option value="{{$cat->id}}"
                                                {{ ($pro->category_id) == ($cat->id) ? "selected" : "" }}>
                                                {{$cat->name}}
                                            </option>;
                                            @endforeach
                                        </select>
                            ...
                            <a class='btn btn-sm btn-warning' href="{{ route('product.edit', $pro->id) }}"><i
                    ...
                    @if($pros->count() <= 0) <p class='lead text-center'><em>Hiện chưa có sản phẩm nào, vui lòng thêm
                            sản phẩm!</em>
                        </p>
                        @endif
                        <div class="pagination justify-content-end">
                            <li class="paginate_button page-item previous"><a href="{{ $pros->url(1) }}"
                                    class="page-link">&laquo;&laquo;</a>
                            </li>
                            {{$pros->links('pagination::bootstrap-4')}}
                            <li class="paginate_button page-item next"><a href="{{ $pros->url($pros->lastPage()) }}"
